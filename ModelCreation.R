...
initial_rf <- randomForest(X16 ~., data = crx_train)
...
rf_pred_train <- predict(RF_final, newdata = crx_train)

rf_pred_test <- predict(RF_final, newdata = crx_test)

confusionMatrix(data=rf_pred_train, reference=crx_train$X16)

confusionMatrix(data=rf_pred_test, reference=crx_test$X16)
